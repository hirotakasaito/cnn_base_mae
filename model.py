...
class ResNetImageEncoder(nn.Module):
    ...
    def forward(self, obs):
        t, b, c, h, w = obs.size()
        _obs = obs.reshape([t*b,c,h,w])
        resnet_output = self.resnet_layer(_obs)
        resnet_output = F.adaptive_avg_pool2d(resnet_output,(1,1))
        x = resnet_output.reshape(t*b,-1)
        x = self.fc1(x)
        x = self.fc2(x)

        if self.dropout > 0:
            x = self.dropout1(x)
        embedded_obs = F.relu(self.fc3(x)).reshape([t,b,self.embedding_size])
...
class Vgg16ImageEncoder(nn.Module):
    ...
    def forward(self, obs):
        t, b, c, h, w = obs.size()
        _obs = obs.reshape([t*b,c,h,w])
        resnet_output = self.vgg16_layer(_obs)
...
class ImageDecoder(nn.Module):
    ...
    def __init__(self, observation_size, state_size, belief_size, embedding_size):
        ...
        self.deconv = nn.Sequential(
            ...
            nn.ELU(),
            nn.ConvTranspose2d(16, 3, 4, stride=4, padding=0), #(N,16,64,64) -> (N,3,256,256)
            nn.Sigmoid()
        )
    ...
    def forward(self, state, belief):
        t,b,_ = state.size()
        hidden = F.elu(self.fc1(torch.cat([state, belief], dim=-1)))
        ...
        observation = observation.reshape([t,b,c,h,w])
        return observation
...
class MultiAttentionNetwork(nn.Module):
    ...
    def forward(self, obs):
        ...
        tb, _, h, w = attn.shape
        self.mask_ = attn
        resnet_output = resnet_output.reshape(t, b, 1, 512, h, w)
        attn = attn.reshape(t,b,self.num_masks,1,h,w)
        resnet_output = resnet_output * attn
        resnet_output = resnet_output.reshape(t*b*self.num_masks,512,h,w)
        resnet_output = F.adaptive_avg_pool2d(resnet_output,(1,1))
        flatten = resnet_output.reshape(t*b,-1)

        if self.dropout > 0:
            x = self.dropout1(flatten)
        embedded_obs = self.fc(flatten)
        embedded_obs = embedded_obs.reshape(t,b,-1)
        if(self.return_img):
            return embedded_obs, obs
        else:
            return embedded_obs,attn
