...
class ImageDecoder224(nn.Module):
    ...
    def __init__(self, observation_size,embedded_obs,  embedding_size):
        ...
        # Deconv layers
        self.deconv = nn.Sequential(
            nn.ConvTranspose2d(64, 32, 2, stride=4, padding=1), #(N,64,8,8) -> (N,32,32,32)
            nn.ELU(),
            nn.ConvTranspose2d(32, 16, 4, stride=4, padding=1), #(N,32,32,32) -> (N,16,64,64)
            nn.ELU(),
            nn.ConvTranspose2d(16, 3, 6, stride=2, padding=0), #(N,16,64,64) -> (N,3,256,256)
            # nn.ConvTranspose2d(16, 3, 2, stride=2, padding=0), #(N,16,64,64) -> (N,3,256,256)
            nn.Sigmoid()
        )
